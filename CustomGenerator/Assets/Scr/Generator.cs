            ...
            switch (hallway.getLineStyle())
            {
                case Line.LineStyle.VertFirst:
                    ...
                    if (dy > 0) // if y up
                    {
                        ...
                        cells[startPos[0], endPos[1]].Exits = 15;
                    }
                    else // if y down
                    {
                    ...
                    if (dx > 0) // if x right
                    {
                        ...
                        cells[endPos[0], endPos[1]].Exits |= (int)InverseExit.Right;
                    }
                    else // if x left
                    {
                        cells[startPos[0], endPos[1]].Exits = 15;
                        ...
                        cells[endPos[0], endPos[1]].Exits |= (int)InverseExit.Left;
                    }
                    break;
                case Line.LineStyle.HorizFirst:
                    ...
                    if (dx > 0)
                    {
                        cells[startPos[0], startPos[1]].Exits |= (int)Exit.Right;
                        for (int x = startPos[0] + 1; x <= endPos[0] - 1; x++)
                        {
                            cells[x, startPos[1]].Exits |= (int)Exit.Left + (int)Exit.Right;
                        }
                        cells[endPos[0], startPos[1]].Exits = 15;
                    }
                    else
                    {
                        cells[startPos[0], startPos[1]].Exits |= (int)Exit.Left;
                        for (int x = startPos[0] - 1; x >= endPos[0] + 1; x--)
                        {
                            cells[x, startPos[1]].Exits |= (int)Exit.Left + (int)Exit.Right;
                        }
                    ...
                    if (dy > 0)
                    {
                        ...
                        cells[endPos[0], startPos[1]].Exits = 15;
                        cells[endPos[0], endPos[1]].Exits |= (int)InverseExit.Up;
                    }
                    else
                    {
                        for (int y = startPos[1] - 1; y >= endPos[1] + 1; y--)
                        ...
                        cells[endPos[0], startPos[1]].Exits = 15;
                        cells[endPos[0], endPos[1]].Exits |= (int)InverseExit.Down;
                    }
                    break;
                default:
        ...
        for (int y = 0; y < HEIGHT; y++)
        {
            for (int x = 0; x < WIDTH; x++)
            {
                ...
                if (currentRoom != null)
                {
                    ...
                    {
                        if (((currentCell.Exits >> spawnOrientation) & 1) == 1)
                        {
                ...
                else
                {
                    ...
                    spawnOrientation = currentCell.getOrientation(ref spawnOrientation);
                    switch (exits)
                    {
                        case 0:
